...
# TRL 1.1.x expects FSDPModule, which is absent in some torch versions.
if not hasattr(_fsdp, "FSDPModule"):
    from torch.distributed.fsdp import FullyShardedDataParallel as _FSDP

    _fsdp.FSDPModule = _FSDP

# Force-disable vLLM path to avoid import-time incompatibilities.
try:
    ...
    def _prepare_inputs(
        ...
        if mode == "train":
            if not hasattr(self, "_step") or self._step is None:
                self._step = 0
            grad_acc_steps = max(1, int(getattr(self.args, "gradient_accumulation_steps", 1)))
            if getattr(self, "_buffered_inputs", None) is None or not isinstance(self._buffered_inputs, dict):
                self._buffered_inputs = {}
            slot = self._step % grad_acc_steps

            if self.state.global_step % self.num_iterations == 0:
                inputs = self._generate_and_score_completions(inputs)
                self._buffered_inputs[slot] = inputs
            else:
                # Newer TRL versions may reset/skip buffer init; regenerate on cache miss.
                if slot not in self._buffered_inputs:
                    inputs = self._generate_and_score_completions(inputs)
                    self._buffered_inputs[slot] = inputs
                else:
                    inputs = self._buffered_inputs[slot]
            self._step += 1
        else:
        ...
        with torch.no_grad():
            with unwrap_model_for_generation(self.model_wrapped, self.accelerator) as unwrapped_model:
                ...
                if generation_batch_size is None or generation_batch_size <= 0:
                    generation_batch_size = prompt_ids.size(0)
