...
def get_tablegpt_questions() -> Dataset:
    cur_path = os.path.dirname(os.path.abspath(__file__))
    tablegpt_file_path = "../dataset/tablegpt_data.json"
    tablegpt_file_path = os.path.join(cur_path, tablegpt_file_path)

    loaded_datasets = load_dataset(
        "json",
        data_files={'train': tablegpt_file_path},
        cache_dir="/home/william/yss/temp"
    )
    ...
    return train_dataset.map(
        lambda x: {
            "prompt": x["msg"],
            "label_gtr": x["label_gtr"],
            "source": x["source"],
            "data_id": x["data_id"],
        }
    )

def get_table_fact_wtq_questions() -> Dataset:
    tablegpt_file_path = "/home/llada/data/llada_sft_shortCoT_train_20k_rl_clean.json"

    loaded_datasets = load_dataset(
        "json",
        data_files={"train": tablegpt_file_path},
        cache_dir="/root/autodl-tmp/.cache/hf_datasets",
    )
    train_dataset = loaded_datasets["train"]
